...
class BakeryPluginTestFix {
    ...
    private fun createMockProject(): Pair<Project, PluginContainer> {
        ...
        val configFile = File("../../site.yml")
        val mockConfigPathProperty = mock<Property<String>> {
            on { get() } doReturn configFile.path
            on { isPresent } doReturn true
        }
        ...
        val mockProjectDirectory = mock<org.gradle.api.file.Directory> {
            on { asFile } doReturn configFile.parentFile.parentFile
        }

        val mockBuildDirectory = mock<DirectoryProperty> {
            on { get() } doReturn mock {
                on { asFile } doReturn File("build")
            }
        }

        val mockProjectLayout = mock<ProjectLayout> {
            on { projectDirectory } doReturn mockProjectDirectory
            on { buildDirectory } doReturn mockBuildDirectory
        }
        ...
        // Mock PluginContainer - THIS IS THE KEY CHANGE
        val mockPluginContainer = mock<PluginContainer>()

        // Mock Project with afterEvaluate support
        val afterEvaluateActions = mutableListOf<Action<Project>>()
        val mockProject = mock<Project> {
            ...
            on { layout } doReturn mockProjectLayout
            on { logger } doReturn mock()
            on { afterEvaluate(any<Action<Project>>()) } doAnswer { invocation ->
                val action = invocation.arguments[0] as Action<Project>
                afterEvaluateActions.add(action)
                // Execute immediately for testing
                action.execute(this.mock)
                null
            }
        }
