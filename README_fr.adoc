...
----
=== 4. Initialiser les Répertoires du Site et de la Maquette (`initSite`)
...
1. Créer un fichier `site.yml` avec les configurations par défaut s'il n'existe pas.
2. S'assurer que `.gitignore` et `.gitattributes` sont correctement configurés pour le plugin.
3. Copier les ressources `site` et `maquette` par défaut dans votre projet, fournissant un modèle de base pour commencer.
...
=== 5. Initialiser la configuration (Optionnel)
